module.exports = app => {
    ...
    const products_by_category = async (req, res) => {
        try {
            ...
            const filter_by_category = await Product.find({ category: { $regex: category, $options: 'i' }}, { _id: 0, __v: 0 })

            return res.status(200).send({ message: `Produtos filtrados pela categoria '${ category }' listados com sucesso!`, data: { filter_by_category }})
        } catch (error) {
            console.log(error)
